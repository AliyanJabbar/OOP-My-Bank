...
class Customer {
    ...
    accountNo;
    pin;
    constructor(fName, lName, age, gender, mobNo, accNo, pin) {
...
class BankAccount {
    ...
    checkBalance(cus, cusBankAcc) {
        console.log(`DEAR ${chalk.green.bold.italic(cus.firstName)} ${chalk.green.bold.italic(cus.lastName)}, YOUR BALANCE IS:${chalk.bold.magenta(`${cusBankAcc.Balance}$`)}`);
    }
    // method to view balance
    viewDetails(customer, bankAcc) {
        console.log(`NAME: ${chalk.green.bold.italic(customer.firstName)} ${chalk.green.bold.italic(customer.lastName)}`);
        console.log(`AGE: ${chalk.green.bold.italic(customer.age)}`);
        console.log(`GENDER: ${chalk.green.bold.italic(customer.gender)}`);
        console.log(`WHATSAPP NUMBER: ${chalk.green.bold.italic(customer.mobileNo)}`);
        console.log(`ACCOUNT NUMBER: ${chalk.green.bold.italic(customer.accountNo)}`);
        console.log(`PIN: ${chalk.green.bold.italic(customer.pin)}`);
    }
    // method to debit(withdraw)
    debit(customer, cusBankAcc, amount) {
        cusBankAcc.Balance -= amount;
        console.log(`DEAR ${chalk.green.bold.italic(customer.firstName)} ${chalk.green.bold.italic(customer.lastName)}, ${chalk.magenta(amount)}$ HAS BEEN DEBITED FROM YOUR BANK ACCOUNT. YOUR NEW BALANCE IS ${chalk.magenta(cusBankAcc.Balance)}$`);
    }
    // method to debit more than 100$
    debitMore(customer, cusBankAcc, amount) {
        cusBankAcc.Balance -= amount + 1;
        console.log(`DEAR ${chalk.green.bold.italic(customer.firstName)} ${chalk.green.bold.italic(customer.lastName)}, ${chalk.magenta(amount)}$ HAS BEEN DEBITED FROM YOUR BANK ACCOUNT. YOUR NEW BALANCE IS ${chalk.magenta(cusBankAcc.Balance)}$`);
    }
    // method to credit(deposit) money 
    credit(customer, cusBankAcc, amount) {
        cusBankAcc.Balance += amount;
        console.log(`DEAR ${chalk.green.bold.italic(customer.firstName)} ${chalk.green.bold.italic(customer.lastName)}, ${chalk.magenta(amount)}$ HAS BEEN CREDITED FROM YOUR BANK ACCOUNT. YOUR NEW BALANCE IS ${chalk.magenta(cusBankAcc.Balance)}$`);
    }
}
        ...
        if (user.input === "CREATE BANK ACCOUNT") {
            let user = await inquirer.prompt([
                ...
                },
                {
                    name: "mobNo",
                    type: "number",
                    message: chalk.yellow("ENTER YOUR WHATSAPP NUMBER(fake number is allowed!):"),
                    validate: (val) => {
                        if (Number.isNaN(val)) {
            ...
            let customer = new Customer(firstName, lastName, user.age, user.gender, user.mobNo, accNo, user.pin);
            bank.addCus(customer);
            bank.addBankAcc(customer);
            console.log(chalk.green.bold.italic("YOUR BANK ACCOUNT HAS BEEN CREATED SUCCESFULLY!"));
            console.log(chalk.green("-".repeat(121)));
            //finding customer through account number because it will be unique
            let findCus = bank.customers.find((cus) => cus.accountNo === customer.accountNo);
        ...
        if (user.input === "CHECK YOUR DETAILS") {
            ...
            if (findCus) {
                let user2 = await inquirer.prompt({
                    name: "pin",
                    type: "number",
                    message: chalk.yellow("INPUT YOUR ACCOUNT'S PIN TO SEE YOUR DETAILS..."),
                });
                if (findCus.pin === user2.pin) {
                    let findAcc = bank.bankAccs.find((bankAcc) => bankAcc.accNo === user.input);
            ...
            if (findCus) {
                ...
                if (findCus.pin === user2.pin) {
                    let user3 = await inquirer.prompt({
                        name: "amount",
                        type: "number",
                        message: chalk.yellow(`HOW MUCH AMOUNT DO YOU WANT TO WITHDRAW? ${chalk.red.bold("NOTE:")} ${chalk.red.italic("$1 WILL BE CHARGED IF YOU WITHDRAW 100$ OR MORE")}`),
                    });
